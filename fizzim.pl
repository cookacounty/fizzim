...
@orig_argv = @ARGV;
&process_options;
$source_file = $ARGV[0] if (!defined($source_file) && defined($ARGV[0]));
...
# Output version data
if ($addversion) {
  $source_file = "" unless defined($source_file);
  &print($indent,"${comm} Generated by Fizzim 2.0\n");
  &print($indent,"${comm} Fizzim version: $fizzimversion\n");
  &print($indent,"${comm} Source diagram: $source_file\n") if ($source_file ne "");
  if ($sourcechecksum && ($source_file ne "")) {
    $checksum = &source_checksum($source_file);
    &print($indent,"${comm} Source checksum: sha256:$checksum\n") if ($checksum ne "");
  }
  &print($indent,"\n");
  if ($be_options) {
...
sub source_checksum {
  my ($file) = @_;
  return "" unless (($file ne "") && (-e $file));
  eval {
    require Digest::SHA;
    Digest::SHA->import(qw(sha256_hex));
  };
  if ($@) {
    &warning(0,"Could not load Digest::SHA for source checksum: $@");
    return "";
  }
  open(my $fh, "<", $file) or do {
    &warning(0,"Could not open source file $file for checksum");
    return "";
  };
  binmode($fh);
...
sub process_options {
  ...
  die unless GetOptions(
    ...
    "statenamechars=s" => \$statename_chars,
    "indexvar=s" => \$indexvar,
    "warnout=s" => \$warnout,
    "nowarn=s" => \@nowarns,
    "version!" => \$version,
    "addversion!" => \$addversion,
    "fizzimversion=s" => \$fizzimversion,
...
sub show_help {
 print STDERR "Syntax:
  ...
  -statenamechars <value> Minimum number of ASCII characters reserved for the simulation state name variable
  ...
  -addversion           Add deterministic Fizzim provenance header to output as comments
  -fizzimversion <version> Version string to print in the provenance header
  -sourcefile <path>    Source diagram name/path to print in the provenance header
  -sourcechecksum       Add SHA-256 checksum of sourcefile to the provenance header
