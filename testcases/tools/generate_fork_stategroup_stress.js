...
function setOutputValues(obj, values, blankUnspecified = false) {
  const defaults = {
    rdy: "0",
    busy: "0",
    "color[100:0]": '"none"',
  ...
  ["rdy", "busy", "color[100:0]", "trigger", "trigger1[1:0]"].forEach((name) => {
    const value = Object.prototype.hasOwnProperty.call(values, name) ? values[name] : (blankUnspecified ? "" : defaults[name]);
    setAttrValue(obj.block, name, value, value === "" ? "GLOBAL_VAR" : "LOCAL");
  });
}

function makeState(template, name, x, y, values = {}, blankUnspecified = false) {
  ...
  setOutputValues(obj, values, blankUnspecified);
  return obj;
}
...
function makeTransition(template, name, start, end, equation, priority, actions = {}) {
  ...
  setOutputValues(obj, actions, true);
  return obj;
}
...
function main() {
  ...
  const states = [
    ...
    makeState(stateTemplate, "S_A0", 420, 140, {}, true),
    makeState(stateTemplate, "S_A1", 620, 140, { "color[100:0]": '"a1"' }, true),
    makeState(stateTemplate, "S_B0", 420, 420, {}, true),
    makeState(stateTemplate, "S_B1", 620, 420, { "color[100:0]": '"b1"' }, true),
  ...
  fs.writeFileSync(outFzm, pre.concat(finalObjects.flatMap((o) => o.block), post).join("\n") + "\n");
  writeReferenceRtl();
  writeTestbench();
}
